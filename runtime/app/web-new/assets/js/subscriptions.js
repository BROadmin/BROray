(function () {
    ...
    function renderWarnings(item) {
        var result = item && item.lastUpdateResult ? item.lastUpdateResult : null;
        var warnings = result && Array.isArray(result.warnings) ? result.warnings : [];

        if (!warnings.length) return "";

        return '<div class="subscription-warning" role="status">' +
            '<strong>Предупреждения обновления</strong>' +
            '<ul>' + warnings.map(function (warning) {
                var message = warning && typeof warning === "object"
