...
BUILD_OUTPUT="/opt/broray/tmp/broray-routes-verify-build-$$.out"
BUILD_ERROR="/opt/broray/tmp/broray-routes-verify-build-$$.err"
PLAN_OUTPUT="/opt/broray/tmp/broray-routes-verify-plan-$$.json"
PLAN_ERROR="/opt/broray/tmp/broray-routes-verify-plan-$$.err"
STATE_NEW=""
...
if ! "$CLI" plan "$bundle_id" >"$PLAN_OUTPUT" 2>"$PLAN_ERROR"; then
    ...
    case "$details" in
        *ROUTES_CONFIG_NDMC_LOCK_FAILED*)
            reason="keenetic_read_busy"
            message="Keenetic временно занят чтением конфигурации. Повторите проверку через несколько секунд."
            ;;
        *ROUTES_CONFIG_NDMC_TIMEOUT*|*ROUTES_CONFIG_RUNNING_CONFIG_READ_FAILED*|*ROUTES_PLAN_RUNNING_CONFIG_READ_FAILED*)
            reason="keenetic_read_failed"
            message="Keenetic не ответил на безопасное чтение конфигурации. Повторите проверку."
            ;;
        *ROUTES_PLAN_RUNNING_CONFIG_UNSUPPORTED*)
            reason="unsupported_route_format"
            message="В конфигурации Keenetic есть статический маршрут неподдерживаемого формата. BROray не продолжит без безопасного разбора."
            ;;
        *ROUTES_PLAN_REGISTRY_INVALID*)
            reason="registry_invalid"
            message="Внутренний реестр маршрутов BROray не прошёл проверку. Маршруты Keenetic не изменялись."
            ;;
        *ROUTES_PLAN_LOCAL_VERSION_MISMATCH*)
            reason="local_version_changed"
            message="Локальные данные набора изменились во время проверки. Повторите подготовку набора."
            ;;
        *откат*|*rollback*)
            reason="rollback_required"
            message="Проверка заблокирована: предыдущий откат маршрутов требует восстановления."
            ;;
    esac
