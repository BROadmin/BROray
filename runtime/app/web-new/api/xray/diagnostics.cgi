...
STATUS_JSON_NORMALIZED="$(
    printf '%s\n' "$STATUS_JSON" |
        jq -c -s 'if length == 1 and (.[0] | type == "object") then .[0] else empty end' 2>/dev/null || :
)"
[ -n "$STATUS_JSON_NORMALIZED" ] || STATUS_JSON_NORMALIZED='{}'
STATUS_JSON="$STATUS_JSON_NORMALIZED"

VALIDATE_JSON="$(
    "$BRORAY" xray validate 2>/dev/null || printf '{}'
)"

VALIDATE_JSON_NORMALIZED="$(
    printf '%s\n' "$VALIDATE_JSON" |
        jq -c -s 'if length == 1 and (.[0] | type == "object") then .[0] else empty end' 2>/dev/null || :
)"
[ -n "$VALIDATE_JSON_NORMALIZED" ] || VALIDATE_JSON_NORMALIZED='{}'
VALIDATE_JSON="$VALIDATE_JSON_NORMALIZED"

BINARY="/opt/broray/runtime/xray"
...
checks_json_normalized="$(
    printf '%s\n' "$checks_json" |
        jq -c -s 'if length == 1 and (.[0] | type == "array") then .[0] else empty end' 2>/dev/null || :
)"
[ -n "$checks_json_normalized" ] || checks_json_normalized='[]'
checks_json="$checks_json_normalized"
