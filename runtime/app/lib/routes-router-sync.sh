...
BRORAY_SYNC_ROLLBACK_MARKER="${BRORAY_SYNC_ROLLBACK_MARKER:-$BRORAY_SYNC_ROUTES/rollback-required.json}"
BRORAY_SYNC_BASELINE_ROUTES=""
BRORAY_SYNC_BASELINE_CONFIG_SHA=""
BRORAY_SYNC_ROLLBACK_VERIFIED=false
BRORAY_SYNC_RESUME_PROGRESS_BACKUP=false
BRORAY_SYNC_VERIFY_BATCH_SIZE="${BRORAY_SYNC_VERIFY_BATCH_SIZE:-16}"
...
{
    command -v broray_routes_config_save_converged >/dev/null 2>&1 || return 1
    BRORAY_ROUTES_CONFIG_NDMC="$BRORAY_SYNC_NDMC"
    export BRORAY_ROUTES_CONFIG_NDMC
    broray_routes_config_save_converged "$BRORAY_SYNC_WORK/save-convergence"
}
...
broray_routes_sync_backup_resume_progress()
{
    local progress_dir progress counter

    [ "$BRORAY_SYNC_RESUMED" = true ] || return 0
    [ -d "$BRORAY_SYNC_LOCAL_BACKUP" ] || return 1
    progress_dir="${BRORAY_ROUTES_PROGRESS_DIR:-$BRORAY_SYNC_ROUTES/operations}"
    progress="$progress_dir/$BRORAY_SYNC_BUNDLE.json"
    counter="$progress_dir/$BRORAY_SYNC_BUNDLE.counter"
    [ -f "$progress" ] && [ ! -L "$progress" ] && [ -s "$progress" ] || return 1
    jq -e --arg bundleId "$BRORAY_SYNC_BUNDLE" '
        .kind == "routes" and .bundleId == $bundleId and
        .running == false and .resumable == true
    ' "$progress" >/dev/null 2>&1 || return 1
    cp -p "$progress" "$BRORAY_SYNC_LOCAL_BACKUP/resume-progress.json" || return 1
    if [ -f "$counter" ] && [ ! -L "$counter" ]; then
        cp -p "$counter" "$BRORAY_SYNC_LOCAL_BACKUP/resume-progress.counter" || return 1
    else
        : >"$BRORAY_SYNC_LOCAL_BACKUP/resume-progress.counter.missing" || return 1
    fi
    BRORAY_SYNC_RESUME_PROGRESS_BACKUP=true
}

broray_routes_sync_restore_resume_progress()
{
    local progress_dir progress counter

    [ "$BRORAY_SYNC_RESUMED" = true ] || return 1
    [ "$BRORAY_SYNC_RESUME_PROGRESS_BACKUP" = true ] || return 1
    [ -s "$BRORAY_SYNC_LOCAL_BACKUP/resume-progress.json" ] || return 1
    progress_dir="${BRORAY_ROUTES_PROGRESS_DIR:-$BRORAY_SYNC_ROUTES/operations}"
    progress="$progress_dir/$BRORAY_SYNC_BUNDLE.json"
    counter="$progress_dir/$BRORAY_SYNC_BUNDLE.counter"
    mkdir -p "$progress_dir" || return 1
    cp -p "$BRORAY_SYNC_LOCAL_BACKUP/resume-progress.json" "$progress.new.$$" &&
        mv -f "$progress.new.$$" "$progress" || return 1
    if [ -f "$BRORAY_SYNC_LOCAL_BACKUP/resume-progress.counter.missing" ]; then
        rm -f "$counter" 2>/dev/null || return 1
    else
        cp -p "$BRORAY_SYNC_LOCAL_BACKUP/resume-progress.counter" "$counter.new.$$" &&
            mv -f "$counter.new.$$" "$counter" || return 1
    fi
    BRORAY_SYNC_PROGRESS_ACTIVE=false
    return 0
}

broray_routes_sync_route_state_equal()
{
    local before after before_canonical after_canonical

    before="$1"
    after="$2"
    before_canonical="$BRORAY_SYNC_WORK/rollback-route-state-before.json"
    after_canonical="$BRORAY_SYNC_WORK/rollback-route-state-after.json"

    jq -S '{
        serializationComplete,
        unknownRouteCount,
        routes:([.routes[]? | {
            network, mask, prefix, destination, interface, gateway, metric, proto,
            automatic, exclusive,
            serialization:{known:.serialization.known, metricExplicit:.serialization.metricExplicit}
        }] | sort_by(.destination, .interface, .gateway, .metric, .proto))
    }' "$before" >"$before_canonical" || return 1
    jq -S '{
        serializationComplete,
        unknownRouteCount,
        routes:([.routes[]? | {
    ...
    }' "$after" >"$after_canonical" || return 1
    cmp -s "$before_canonical" "$after_canonical"
}

broray_routes_sync_config_canonicalize()
{
    local input output
...
broray_routes_sync_non_route_config_canonicalize()
{
    local input output
...
broray_routes_sync_parse_raw_snapshot()
{
    local input output fixture_before fixture_was_set rc
    ...
    fixture_before=""
    fixture_was_set=false
    if [ "${BRORAY_ROUTES_CONFIG_FIXTURE+x}" = x ]; then
        fixture_before="$BRORAY_ROUTES_CONFIG_FIXTURE"
        fixture_was_set=true
    fi

    BRORAY_ROUTES_CONFIG_FIXTURE="$input"
    if broray_routes_config_fetch "$output"; then rc=0; else rc=$?; fi

    if [ "$fixture_was_set" = true ]; then
        BRORAY_ROUTES_CONFIG_FIXTURE="$fixture_before"
    else
        unset BRORAY_ROUTES_CONFIG_FIXTURE
    fi

    [ "$rc" -eq 0 ] || return "$rc"
    broray_routes_config_serialization_complete "$output"
}

broray_routes_sync_config_matches_baseline()
{
    local reference_snapshot running parsed startup startup_parsed
    local running_canonical startup_canonical digest ndmc_bin

    reference_snapshot="$1"
    running="$BRORAY_SYNC_WORK/rollback-running-config.txt"
    parsed="$BRORAY_SYNC_WORK/rollback-running-exact.json"
    startup="$BRORAY_SYNC_WORK/rollback-startup-config.txt"
    startup_parsed="$BRORAY_SYNC_WORK/rollback-startup-exact.json"
    running_canonical="$BRORAY_SYNC_WORK/rollback-running.non-route.canonical"
    startup_canonical="$BRORAY_SYNC_WORK/rollback-startup.non-route.canonical"

    [ -s "$reference_snapshot" ] || return 1

    case "$BRORAY_SYNC_NDMC" in
        */*) ndmc_bin="$BRORAY_SYNC_NDMC" ;;
        *) ndmc_bin="$(command -v "$BRORAY_SYNC_NDMC" 2>/dev/null || true)" ;;
    esac
    [ -n "$ndmc_bin" ] && [ -x "$ndmc_bin" ] || return 1
    broray_routes_config_ndmc_capture \
        "$ndmc_bin" "show running-config" "$running" \
        "$BRORAY_SYNC_WORK/rollback-running-config.err" 15 || return 1
    broray_routes_sync_parse_raw_snapshot "$running" "$parsed" || return 1
    broray_routes_sync_route_state_equal "$reference_snapshot" "$parsed" || return 1
    broray_routes_config_ndmc_capture \
        "$ndmc_bin" "more startup-config" "$startup" \
        "$BRORAY_SYNC_WORK/rollback-startup-config.err" 15 || return 1
    broray_routes_sync_parse_raw_snapshot "$startup" "$startup_parsed" || return 1
    broray_routes_sync_route_state_equal "$reference_snapshot" "$startup_parsed" || return 1
    broray_routes_sync_non_route_config_canonicalize "$running" "$running_canonical" || return 1
    broray_routes_sync_non_route_config_canonicalize "$startup" "$startup_canonical" || return 1
    cmp -s "$running_canonical" "$startup_canonical" || return 1
    digest="$(sha256sum "$running_canonical" | awk 'NR == 1 {print $1}')"
    [ "$digest" = "$BRORAY_SYNC_BASELINE_CONFIG_SHA" ]
}

broray_routes_sync_capture_baseline()
{
    local plan_snapshot baseline_snapshot running startup startup_snapshot digest ndmc_bin
    local running_canonical startup_canonical

    plan_snapshot="$BRORAY_SYNC_WORK/running-config.json"
    baseline_snapshot="$BRORAY_SYNC_WORK/running-config-baseline.json"
    running="$BRORAY_SYNC_WORK/running-config-before.txt"
    startup="$BRORAY_SYNC_WORK/startup-config-before.txt"
    startup_snapshot="$BRORAY_SYNC_WORK/startup-config-baseline.json"
    [ -s "$plan_snapshot" ] || return 1
    ...
    broray_routes_config_ndmc_capture \
        "$ndmc_bin" "show running-config" "$running" \
        "$BRORAY_SYNC_WORK/running-config-before.err" 15 || return 1
    broray_routes_sync_parse_raw_snapshot "$running" "$baseline_snapshot" || return 1
    broray_routes_sync_route_state_equal "$plan_snapshot" "$baseline_snapshot" || return 1
    broray_routes_config_ndmc_capture \
        "$ndmc_bin" "more startup-config" "$startup" \
        "$BRORAY_SYNC_WORK/startup-config-before.err" 15 || return 1
    broray_routes_sync_parse_raw_snapshot "$startup" "$startup_snapshot" || return 1
    broray_routes_sync_route_state_equal "$baseline_snapshot" "$startup_snapshot" || return 1
    running_canonical="$BRORAY_SYNC_WORK/running-config-before.non-route.canonical"
    startup_canonical="$BRORAY_SYNC_WORK/startup-config-before.non-route.canonical"
    broray_routes_sync_non_route_config_canonicalize "$running" "$running_canonical" || return 1
    broray_routes_sync_non_route_config_canonicalize "$startup" "$startup_canonical" || return 1
    cmp -s "$running_canonical" "$startup_canonical" || return 1
    ...
    BRORAY_SYNC_BASELINE_ROUTES="$baseline_snapshot"
    BRORAY_SYNC_BASELINE_CONFIG_SHA="$digest"
    printf '%s\n' "$digest" >"$BRORAY_SYNC_WORK/baseline.sha256" || return 1
}

broray_routes_sync_rollback_marker_write()
{
    local message temp now

    message="$1"
    now="$(broray_routes_sync_now)"
    temp="$BRORAY_SYNC_ROLLBACK_MARKER.new.$$"
    mkdir -p "$(dirname "$BRORAY_SYNC_ROLLBACK_MARKER")" || return 1
    jq -n \
        --arg bundleId "${BRORAY_SYNC_BUNDLE:-}" \
        --arg transaction "${BRORAY_SYNC_TRANSACTION:-}" \
        --arg baselineSha256 "${BRORAY_SYNC_BASELINE_CONFIG_SHA:-}" \
        --arg message "$message" \
        --arg createdAt "$now" '
        {
            schemaVersion:1,
            kind:"routes-rollback-required",
            bundleId:$bundleId,
            transaction:$transaction,
            baselineSha256:(if $baselineSha256 == "" then null else $baselineSha256 end),
            message:$message,
            blocksNewMutations:true,
            createdAt:$createdAt
        }
    ' >"$temp" || { rm -f "$temp"; return 1; }
    chmod 600 "$temp" 2>/dev/null || true
    mv -f "$temp" "$BRORAY_SYNC_ROLLBACK_MARKER"
}

broray_routes_sync_rollback_router()
{
    local tab kind key network prefix mask interface metric out err rc
    local reverse failed changed snapshot identity_count attribute_count index final_snapshot
    local verification_attempt verification_limit verification_ok

    [ "$BRORAY_SYNC_ROUTER_CHANGED" = true ] || return 0
    broray_routes_static_write_policy_check || return 1
    tab="$(printf '\t')"
    failed=false
    changed=false
    index=0

    [ -n "$BRORAY_SYNC_BASELINE_ROUTES" ] &&
    [ -s "$BRORAY_SYNC_BASELINE_ROUTES" ] &&
    [ -n "$BRORAY_SYNC_BASELINE_CONFIG_SHA" ] || return 1
    snapshot="$BRORAY_SYNC_WORK/rollback-current.json"
    broray_routes_config_fetch "$snapshot" &&
        broray_routes_config_serialization_complete "$snapshot" || return 1

    if [ -s "$BRORAY_SYNC_DELETED_FILE" ]; then
        reverse="$BRORAY_SYNC_WORK/rollback-deleted.reverse.tsv"
        awk '{line[NR]=$0} END {for (n=NR; n>=1; n--) print line[n]}' \
            "$BRORAY_SYNC_DELETED_FILE" >"$reverse" || return 1
        while IFS="$tab" read -r kind key network prefix mask interface metric; do
            [ -n "$network" ] || continue
            index=$((index + 1))
            identity_count="$(broray_routes_config_identity_count \
                "$snapshot" "$network" "$prefix" "$interface" "0.0.0.0")"
            attribute_count="$(broray_routes_config_attribute_count \
                "$snapshot" "$network" "$prefix" "$interface" "0.0.0.0" "$metric")"
            if [ "$identity_count" = 1 ] && [ "$attribute_count" = 1 ]; then
                continue
            fi
    ...
    if [ -s "$BRORAY_SYNC_ADDED_FILE" ]; then
        reverse="$BRORAY_SYNC_WORK/rollback-added.reverse.tsv"
        awk '{line[NR]=$0} END {for (n=NR; n>=1; n--) print line[n]}' \
            "$BRORAY_SYNC_ADDED_FILE" >"$reverse" || return 1
        while IFS="$tab" read -r kind key network prefix mask interface metric; do
            [ -n "$network" ] || continue
            index=$((index + 1))
            identity_count="$(broray_routes_config_identity_count \
    ...
    if [ "$changed" = true ]; then
        # A save acknowledgement may time out even though Keenetic converges a
        # moment later.  The authoritative decision below is the bounded exact
        # running/startup comparison against the captured baseline.
        broray_routes_sync_save_config >/dev/null 2>&1 || true
    fi

    final_snapshot="$BRORAY_SYNC_WORK/rollback-final.json"
    verification_limit="${BRORAY_ROUTES_CONFIG_CONVERGENCE_ATTEMPTS:-12}"
    case "$verification_limit" in ''|*[!0-9]*) verification_limit=12 ;; esac
    [ "$verification_limit" -ge 1 ] 2>/dev/null || verification_limit=12
    [ "$verification_limit" -le 30 ] 2>/dev/null || verification_limit=30
    verification_attempt=1
    verification_ok=false
    while [ "$verification_attempt" -le "$verification_limit" ]; do
        if broray_routes_config_fetch "$final_snapshot" &&
           broray_routes_config_serialization_complete "$final_snapshot" &&
           broray_routes_sync_route_state_equal "$BRORAY_SYNC_BASELINE_ROUTES" "$final_snapshot" &&
           broray_routes_sync_config_matches_baseline "$final_snapshot"
        then
            verification_ok=true
            break
        fi
        verification_attempt=$((verification_attempt + 1))
        [ "$verification_attempt" -gt "$verification_limit" ] || sleep 1
    done
    ...
    if [ "$failed" = false ]; then
        BRORAY_SYNC_ROLLBACK_VERIFIED=true
        rm -f "$BRORAY_SYNC_ROLLBACK_MARKER" 2>/dev/null || return 1
    fi
    [ "$failed" = false ]
}
...
broray_routes_sync_verify_batch()
{
    local expected batch_file tag attempts attempt snapshot all_ok tab
    local kind key network prefix mask interface metric identity_count attribute_count
    ...
    attempts="${BRORAY_ROUTES_CONFIG_CONVERGENCE_ATTEMPTS:-12}"
    ...
    while [ "$attempt" -le "$attempts" ]; do
        snapshot="$BRORAY_SYNC_WORK/$tag.$attempt.json"
        ...
        then
            ...
            while IFS="$tab" read -r kind key network prefix mask interface metric; do
                ...
                if [ "$expected" = present ]; then
                    [ "$identity_count" = 1 ] && [ "$attribute_count" = 1 ] || all_ok=false
                else
                    [ "$identity_count" = 0 ] || all_ok=false
                fi
            done <"$batch_file"
...
{
    local message abort_exit_code rollback_ok
    message="$*"
    abort_exit_code="${BRORAY_SYNC_ABORT_EXIT_CODE:-1}"
    case "$abort_exit_code" in
        1|129|130|143) ;;
        *) abort_exit_code=1 ;;
    esac
    trap - EXIT HUP INT TERM
    if [ "$BRORAY_SYNC_ROLLING_BACK" = false ]; then
        ...
        if [ "$rollback_ok" = true ] && [ "$BRORAY_SYNC_RESUMED" = true ]; then
            if broray_routes_sync_restore_resume_progress; then
                broray_routes_sync_transaction_write \
                    "resume_attempt_rolled_back" \
                    "$message Попытка продолжения отменена; прежнее resumable-состояние восстановлено." \
                    >/dev/null 2>&1 || true
            else
                rollback_ok=false
                message="$message Маршруты восстановлены, но resumable-состояние не восстановлено."
            fi
...
broray_routes_sync_trap()
{
    ...
    BRORAY_SYNC_ABORT_EXIT_CODE="$trap_status"
...
broray_routes_sync_interface_check()
{
    ...
    # Do not assign undocumented identity semantics to an `id:` field.
...
broray_routes_sync_build_plan_core()
{
    ...
    local unknown_count unknown_reasons
    ...
    for file in "$config" "$bundles" "$plan" "$registry" "$bundle_registry" "$state"; do
        [ -r "$file" ] || {
            printf 'ROUTES_PLAN_INPUT_UNAVAILABLE:%s\n' "$(basename "$file")" >&2
            return 1
        }
    done
    ...
    [ "$(jq -r '.downloadedVersion.contentSha256 // empty' "$state")" = "$(jq -r '.contentSha256 // empty' "$plan")" ] || {
        printf '%s\n' 'ROUTES_PLAN_LOCAL_VERSION_MISMATCH' >&2
        return 1
    }
    ...
    broray_routes_config_fetch "$actual" || {
        printf '%s\n' 'ROUTES_PLAN_RUNNING_CONFIG_READ_FAILED' >&2
        return 1
    }
    if ! broray_routes_config_serialization_complete "$actual"; then
        unknown_count="$(jq -r '.unknownRouteCount // 0' "$actual" 2>/dev/null || printf 0)"
        unknown_reasons="$(jq -r '[.routes[]? | select(.serialization.known == false) | .serialization.reason] | unique | join(",")' "$actual" 2>/dev/null || true)"
        printf 'ROUTES_PLAN_RUNNING_CONFIG_UNSUPPORTED count=%s reasons=%s\n' \
            "$unknown_count" "$unknown_reasons" >&2
        return 1
    fi
    now="$(broray_routes_sync_now)"
    ...
    jq -n \
        ...
        [
            $p.routes[] as $r |
            (identity_matches($r)) as $identity |
            (attribute_matches($r)) as $attributes |
            (target_matches($r)) as $target |
            ([$g.routes[]? | select(.key == $r.key)]) as $registered |
            {
                route: $r,
                status: (
                    if (($target | length) > ($identity | length)) or (($identity | length) > 1) or (($registered | length) > 1) then "conflict"
                    elif ($identity | length) == 1 and ($attributes | length) != 1 then "conflict"
                    elif ($identity | length) == 1 then
        ...
        [
            ...
            if ($registered | length) != 1 then
                ...
                (identity_matches({network: $route.network, prefix: $route.prefix})) as $identity |
                ...
                (target_matches({network: $route.network, prefix: $route.prefix})) as $target |
                {
                    ...
                    status: (
                        if (($target | length) > ($identity | length)) or (($identity | length) > 1) then "conflict"
                        elif ($identity | length) == 1 and ($attributes | length) != 1 then "conflict"
                        elif ($remaining | length) > 0 then
                            if ($identity | length) == 1 then "shared_keep"
                            elif ($target | length) == 0 then "shared_restore"
                            else "conflict"
                            end
                        else
                            if ($identity | length) == 1 then "delete"
...
broray_routes_sync_plan()
{
    ...
    if [ -e "$BRORAY_SYNC_ROLLBACK_MARKER" ] || [ -L "$BRORAY_SYNC_ROLLBACK_MARKER" ]; then
        broray_routes_sync_abort \
            "Предыдущий откат маршрутов не подтверждён; новые изменения заблокированы до восстановления."
    fi
    ...
    broray_routes_sync_ensure_global_registry || broray_routes_sync_abort "ROUTES_PLAN_REGISTRY_INVALID: не удалось восстановить или проверить общий реестр маршрутов."
...
broray_routes_sync_apply()
{
    ...
    local kind batch_size batch_count batch_file batch_sequence
    ...
    command -v broray_routes_static_write_policy_check >/dev/null 2>&1 ||
        broray_routes_sync_abort "Политика записи статических маршрутов недоступна."
    broray_routes_static_write_policy_check ||
        broray_routes_sync_abort "Запись статических маршрутов не разрешена физически привязанным R14C01-профилем."
    ...
    : >"$BRORAY_SYNC_ADDED_FILE"
    : >"$BRORAY_SYNC_DELETED_FILE"
    trap 'broray_routes_sync_trap' EXIT
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    ...
    broray_routes_sync_save_config ||
        broray_routes_sync_abort "Не удалось сохранить текущую конфигурацию Keenetic перед установкой маршрутов."
    ...
    jq -e '.canApply == true' "$sync_plan" >/dev/null 2>&1 || broray_routes_sync_abort "План содержит конфликты или внешние маршруты на управляемом ProxyN."

    broray_routes_sync_capture_baseline ||
        broray_routes_sync_abort "Running/startup-config не совпадают до операции; изменения не начаты."
    ...
    if [ "${BRORAY_SYNC_TEST_MODE:-0}" = "1" ] &&
       [ "${BRORAY_SYNC_TEST_FAIL_AT_MIDPOINT:-0}" = "1" ]
    then
        [ "$remaining_total" -gt 1 ] ||
            broray_routes_sync_abort "Тестовая операция слишком мала для midpoint failpoint."
        BRORAY_SYNC_TEST_FAIL_AFTER=$(((remaining_total + 1) / 2))
    fi
    ...
    broray_routes_sync_backup_resume_progress ||
        broray_routes_sync_abort "Не удалось сохранить resumable-состояние перед попыткой продолжения."
    ...
    tab="$(printf '\t')"
    batch_size="$BRORAY_SYNC_VERIFY_BATCH_SIZE"
    case "$batch_size" in ''|*[!0-9]*) batch_size=16 ;; esac
    [ "$batch_size" -ge 1 ] 2>/dev/null || batch_size=16
    [ "$batch_size" -le 128 ] 2>/dev/null || batch_size=128
    batch_sequence=0
    batch_count=0
    batch_file="$BRORAY_SYNC_WORK/add-verify-batch.tsv"
    : >"$batch_file"
    ...
    while IFS="$tab" read -r kind key network prefix mask interface metric; do
        ...
        printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
            "$kind" "$key" "$network" "$prefix" "$mask" "$interface" "$metric" \
            >>"$BRORAY_SYNC_ADDED_FILE" ||
            broray_routes_sync_abort "Не удалось записать rollback intent для $network/$prefix."
        broray_routes_sync_ndmc "ip route $network $mask $interface $metric" "$out" "$err" 10 || rc=$?
        [ "$rc" -eq 0 ] || broray_routes_sync_abort "Не удалось добавить маршрут $network/$prefix."
        printf '%s\t%s\t%s\t%s\t%s\t%s\t%s\n' \
            "$kind" "$key" "$network" "$prefix" "$mask" "$interface" "$metric" \
            >>"$batch_file" ||
            broray_routes_sync_abort "Не удалось записать batch-проверку для $network/$prefix."
        ...
        if [ "${BRORAY_SYNC_TEST_MODE:-0}" = "1" ] &&
           [ "${BRORAY_SYNC_TEST_FAIL_AFTER:-0}" = "$processed_count" ]
        then
            broray_routes_sync_abort \
                "Принудительная тестовая ошибка после $processed_count из $progress_total маршрутов."
        fi
        if [ "$batch_count" -ge "$batch_size" ]; then
            batch_sequence=$((batch_sequence + 1))
            broray_routes_sync_verify_batch \
                present "$batch_file" "add-batch-$batch_sequence" ||
                broray_routes_sync_abort "Пакетное добавление маршрутов не сошлось в running-config."
            : >"$batch_file"
            batch_count=0
    ...
    if [ "$batch_count" -gt 0 ]; then
        ...
        broray_routes_sync_verify_batch \
            present "$batch_file" "add-batch-$batch_sequence" ||
            broray_routes_sync_abort "Итоговое пакетное добавление маршрутов не сошлось в running-config."
        : >"$batch_file"
        batch_count=0
        broray_routes_sync_stop_if_requested "$sync_plan"
    fi
    ...
    batch_sequence=0
    batch_file="$BRORAY_SYNC_WORK/delete-verify-batch.tsv"
    : >"$batch_file"
    ...
    while IFS="$tab" read -r kind key network prefix mask interface metric; do
        ...
        if [ "$batch_count" -ge "$batch_size" ]; then
            batch_sequence=$((batch_sequence + 1))
            broray_routes_sync_verify_batch \
                absent "$batch_file" "delete-batch-$batch_sequence" ||
                broray_routes_sync_abort "Пакетное удаление маршрутов не сошлось в running-config."
            : >"$batch_file"
            batch_count=0
            broray_routes_sync_stop_if_requested "$sync_plan"
        fi
    ...
    if [ "$batch_count" -gt 0 ]; then
        ...
        broray_routes_sync_verify_batch \
            absent "$batch_file" "delete-batch-$batch_sequence" ||
            broray_routes_sync_abort "Итоговое пакетное удаление маршрутов не сошлось в running-config."
        : >"$batch_file"
        batch_count=0
        broray_routes_sync_stop_if_requested "$sync_plan"
    fi
    ...
    jq -n -e \
        ...
        def present_with_attributes($r):
            [
                ...
                select(
                    ...
                    ((.gateway // "0.0.0.0") == "0.0.0.0") and
                    .metric == ($r.metric // 1200) and
                    (((.proto // "static") | ascii_downcase) == "static")
                )
            ] |
            length;
        def identity_count($r):
            [
                $a.routes[]? |
                select(
                    .serialization.known == true and
        ...
        (all($p.desired[]; identity_count(.route) == 1 and present_with_attributes(.route) == 1)) and
        (all(
            $p.obsolete[] |
            select(.status == "shared_keep" or .status == "shared_restore");
            identity_count(.route) == 1 and present_with_attributes(.route) == 1
        )) and
        (all(
            $p.obsolete[] |
            select(.status == "delete" or .status == "already_absent");
            identity_count(.route) == 0
        ))
    ' >/dev/null 2>&1 ||
