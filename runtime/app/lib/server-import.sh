    ...
    case "$source_type" in
        ...
        manual)
            ...
            while [ "$attempt" -lt 32 ]; do
                nonce="$(hexdump -n 4 -e '4/1 "%02x"' /dev/urandom 2>/dev/null)"
                case "$nonce" in ????????) ;; *) broray_die "не удалось получить случайный идентификатор сервера" ;; esac
                candidate_id="manual-$timestamp-$nonce"
                if [ ! -e "$BRORAY_SERVERS/$candidate_id.json" ] && [ ! -L "$BRORAY_SERVERS/$candidate_id.json" ]; then
                    printf '%s\n' "$candidate_id"
                    return 0
                fi
                attempt=$((attempt + 1))
            done
            broray_die "не удалось выделить уникальный идентификатор сервера"
            ;;
...
broray_server_save_parsed() {
    ...
    [ -n "$BRORAY_UUID" ] || broray_die "парсер не вернул UUID"
    ...
    printf '%s' "$BRORAY_EXTRA" | jq -e 'type == "object"' >/dev/null 2>&1 || broray_die "параметр extra содержит неправильный JSON"
    printf '%s' "$BRORAY_ALPN" | jq -e 'type == "array"' >/dev/null 2>&1 || broray_die "VLESS ALPN имеет неправильный формат"

    mkdir -p "$BRORAY_SERVERS" "$BRORAY_TMP"
    server_file="$(broray_server_path "$server_id")" || return 1
    [ ! -e "$server_file" ] && [ ! -L "$server_file" ] || broray_die "сервер с идентификатором $server_id уже существует"
    temporary_file="$BRORAY_TMP/server-import.$$.json"

    jq -n \
        --arg id "$server_id" --arg name "$BRORAY_NAME" --arg uri "$original_uri" \
        --arg address "$BRORAY_ADDRESS" --argjson port "$BRORAY_PORT" --arg uuid "$BRORAY_UUID" \
        --arg encryption "$BRORAY_ENCRYPTION" --arg flow "${BRORAY_FLOW:-}" --arg network "$BRORAY_NETWORK" --arg security "$BRORAY_SECURITY" \
        --arg sourceType "$source_type" --arg sourceId "$source_id" --argjson sourceIndex "$source_index" \
        --arg sni "$BRORAY_SNI" --arg fingerprint "$BRORAY_FP" --argjson allowInsecure "$BRORAY_ALLOW_INSECURE" \
        --argjson alpn "$BRORAY_ALPN" --arg publicKey "$BRORAY_PBK" --arg shortId "$BRORAY_SID" --arg spiderX "$BRORAY_SPX" \
        --arg host "$BRORAY_HOST" --arg path "$BRORAY_PATH" --arg serviceName "$BRORAY_SERVICE_NAME" \
        --arg mode "$BRORAY_MODE" --arg headerType "$BRORAY_HEADER_TYPE" --argjson extra "$BRORAY_EXTRA" '
        {
          schemaVersion:2,id:$id,name:$name,
          source:(if $sourceType=="subscription" then {type:$sourceType,subscriptionId:$sourceId,nodeIndex:$sourceIndex} else {type:$sourceType} end),
          uri:$uri,protocol:"vless",address:$address,port:$port,uuid:$uuid,encryption:$encryption,
          flow:(if $flow=="" then null else $flow end),
          network:$network,security:$security,
          tls:{serverName:$sni,fingerprint:$fingerprint,allowInsecure:$allowInsecure,alpn:$alpn},
          reality:{serverName:$sni,fingerprint:$fingerprint,publicKey:$publicKey,shortId:$shortId,spiderX:$spiderX},
...
broray_server_import__shadowed_legacy_1_unused() {
    original_uri="$1"
    source_type="${2:-manual}"
    source_id="${3:-}"
    source_index="${4:-0}"
    ...
    case "$protocol" in
        ...
        trojan)
            ...
            ;;
        shadowsocks)
            broray_server_import_shadowsocks \
                "$original_uri" \
                "$source_type" \
                "$source_id" \
                "$source_index"
...
broray_server_save_shadowsocks()
{
    original_uri="$1"
    server_id="$2"
    source_type="$3"
    ...
    broray_server_validate_id "$server_id"
    [ -n "$BRORAY_METHOD" ] ||
        broray_die "парсер Shadowsocks не вернул метод шифрования"
    [ -n "$BRORAY_PASSWORD" ] ||
        broray_die "парсер Shadowsocks не вернул пароль"
    [ -n "$BRORAY_ADDRESS" ] ||
        broray_die "парсер Shadowsocks не вернул адрес"
    case "$BRORAY_PORT" in
    ...
    [ ! -e "$server_file" ] && [ ! -L "$server_file" ] ||
        broray_die "сервер с идентификатором $server_id уже существует"
    temporary_file="$BRORAY_TMP/server-import.$$.json"
    ...
    jq -n \
        ...
        broray_die "не удалось создать файл Shadowsocks-сервера"

    broray_server_validate "$temporary_file"
    mv "$temporary_file" "$server_file" ||
        broray_die "не удалось сохранить Shadowsocks-сервер"
    chmod 600 "$server_file"
    printf '%s\n' "$server_file"
}
...
broray_server_import_shadowsocks()
{
    ...
    . "$BRORAY_BASE/lib/parser-shadowsocks.sh"
    broray_parse_shadowsocks "$original_uri"
    ...
    server_file="$(
        broray_server_save_shadowsocks \
            "$original_uri" "$server_id" "$source_type" \
    ...
    echo "ID: $server_id"
    echo "Название: $BRORAY_NAME"
    echo "Протокол: Shadowsocks"
    echo "Адрес: $BRORAY_ADDRESS:$BRORAY_PORT"
    echo "Метод: $BRORAY_METHOD"
    echo "Файл: $server_file"
}
# BROray protocol dispatcher override with HY2 and Shadowsocks support
broray_server_import__shadowed_legacy_2_unused()
{
    original_uri="$1"
    source_type="${2:-manual}"
    source_id="${3:-}"
