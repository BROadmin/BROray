...
BRORAY_ROUTES_ROUTER_EXPORT_INTERFACE="${BRORAY_ROUTES_ROUTER_EXPORT_INTERFACE:-}"
...
{
    broray_route_resource_acquire "$BRORAY_ROUTES_ROUTER_EXPORT_LOCK" "export" "${BRORAY_ROUTES_ROUTER_EXPORT_BUNDLE:-}" || return $?
    BRORAY_ROUTES_ROUTER_EXPORT_LOCK_TOKEN="$BRORAY_ROUTE_RESOURCE_TOKEN"
    return 0
}

broray_routes_router_export_lock_release()
{
    broray_route_resource_release "$BRORAY_ROUTES_ROUTER_EXPORT_LOCK" "${BRORAY_ROUTES_ROUTER_EXPORT_LOCK_TOKEN:-}" || return $?
    BRORAY_ROUTES_ROUTER_EXPORT_LOCK_TOKEN=''
    return 0
}
...
broray_routes_router_export_static_conflicts__shadowed_legacy_1_unused()
{
...
broray_routes_router_export_exact_count__shadowed_legacy_1_unused()
{
...
broray_routes_router_export_verify_expected_rci__shadowed_legacy_1_unused()
{
    ...
    jq -n \
        ...
        $plan[0] as $p |
        $actual[0] as $a |
        def identity_count($destination):
            [
                $a.routes[]? |
                select(
        ...
        [
            ...
            (identity_count($destination)) as $identity_count |
            ([
                $a.routes[]? |
                select(
                    .destination == $destination and
            ...
            ] | length) as $attribute_count |
            {identity:$identity_count, attributes:$attribute_count}
        ] as $counts |
        all($counts[]; .identity == 1 and .attributes == 1)
    ' | grep -qx 'true'
}
...
broray_routes_router_export_classify__shadowed_legacy_1_unused()
{
    ...
    jq -n \
        ...
        [
            ...
            [
                $a.routes[]? |
                select(.serialization.known == true and .destination == $destination)
            ] as $matches |
            [
                $matches[] |
...
broray_routes_router_export_build_managed_json()
{
    ...
    local family network prefix mask target gateway metric automatic comment
    ...
    while IFS="$tab" read -r \
        key family network prefix mask target gateway metric automatic comment
    do
        [ -n "$key" ] || continue
...
{
    local created_file reverse_file network mask key prefix
    local output error tab snapshot identity_count attribute_count failed changed
    ...
    do
        ...
        identity_count="$(broray_routes_config_identity_count \
            "$snapshot" "$network" "$prefix" "$BRORAY_ROUTES_ROUTER_EXPORT_INTERFACE" "0.0.0.0")"
        attribute_count="$(broray_routes_config_attribute_count \
            "$snapshot" "$network" "$prefix" "$BRORAY_ROUTES_ROUTER_EXPORT_INTERFACE" \
            "0.0.0.0" "$BRORAY_ROUTES_ROUTER_EXPORT_METRIC")"
        ...
        if broray_routes_router_export_ndmc \
           ...
           broray_routes_config_wait_attribute_state \
               absent "$network" "$prefix" "$BRORAY_ROUTES_ROUTER_EXPORT_INTERFACE" \
               "0.0.0.0" "$BRORAY_ROUTES_ROUTER_EXPORT_METRIC" \
               "$BRORAY_ROUTES_ROUTER_EXPORT_ACTIVE_WORK/rollback-state"
        then
            changed=true
        else
            failed=true
    ...
    if [ "$changed" = true ]; then
        BRORAY_ROUTES_CONFIG_NDMC="$BRORAY_ROUTES_ROUTER_EXPORT_NDMC"
        export BRORAY_ROUTES_CONFIG_NDMC
        broray_routes_config_save_converged \
            "$BRORAY_ROUTES_ROUTER_EXPORT_ACTIVE_WORK/rollback-save" || failed=true
    fi
    [ "$failed" = false ]
}
...
broray_routes_router_export_run__shadowed_legacy_1_unused()
{
    ...
    command -v broray_routes_static_write_policy_check >/dev/null 2>&1 ||
        broray_routes_router_export_error \
            "Политика записи статических маршрутов недоступна."
    broray_routes_static_write_policy_check ||
        broray_routes_router_export_error \
            "Запись статических маршрутов не разрешена физически привязанным R14C01-профилем."
    ...
    case "$managed_interface" in
        Proxy[0-9]*) ;;
        *) broray_routes_router_export_error "Некорректный управляемый интерфейс Keenetic." ;;
    esac
    case "${managed_interface#Proxy}" in
        ''|*[!0-9]*) broray_routes_router_export_error "Некорректный управляемый интерфейс Keenetic." ;;
    esac
    ...
    trap 'broray_routes_router_export_cleanup' EXIT
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
    ...
    # The query is scoped to the owner-validated CLI token.  R14C01 does not
    # infer that an undocumented `id:` field equals that token.
    ...
    broray_routes_config_serialization_complete "$rci_before" ||
        broray_routes_router_export_error \
    ...
    do
        ...
        # Persist rollback intent before ndmc.  Cleanup re-reads the complete
        # running configuration, so a command that never took effect is a
        # safe no-op while a command interrupted after applying is reversible.
        printf '%s\t%s\t%s\t%s\n' \
            "$network" \
            "$mask" \
            "$key" \
            ...
            >>"$created_file" ||
            broray_routes_router_export_error \
                "Не удалось записать rollback intent для $destination."
    ...
    broray_routes_config_serialization_complete "$rci_after" ||
        broray_routes_router_export_error \
    ...
    BRORAY_ROUTES_CONFIG_NDMC="$BRORAY_ROUTES_ROUTER_EXPORT_NDMC"
    export BRORAY_ROUTES_CONFIG_NDMC
    broray_routes_config_save_converged "$work/save-convergence" ||
        broray_routes_router_export_error \
...
broray_routes_router_export_fetch_rci__shadowed_legacy_2_unused()
{
    ...
    cp -f "$raw_file" /opt/broray/tmp/broray-rci-export-last.json 2>/dev/null || true
    cp -f "$error_file" /opt/broray/tmp/broray-rci-export-last.error 2>/dev/null || true
    return 1
}
...
broray_routes_router_export_classify()
{
    ...
    jq -n \
        ...
        [
            ...
            [
                $identity_matches[] |
                select(.serialization.metricExplicit == true and .metric == $route.metric)
            ] as $attribute_matches |
            [
                $matches[] |
                select(.interface != $p.targetInterface)
            ] as $other_matches |
            ...
            (
                if $unknown_count > 0 then
                    ...
                    if ($attribute_matches | length) != 1 then
                        "conflict"
                    elif ($owned_entries | length) == 1 then
                        "managed_existing"
                    else
                        "external_existing"
                    end
        ...
        .canExport = (
            ...
            (.summary.unknownSerialization == 0)
        )
    ' >"$output_file"
}
...
broray_routes_router_export_run()
{
    ...
    do
        ...
        export BRORAY_ROUTES_CONFIG_NDMC
        broray_routes_config_wait_attribute_state \
            present "$network" "${destination#*/}" "$managed_interface" \
            "0.0.0.0" "$BRORAY_ROUTES_ROUTER_EXPORT_METRIC" \
            "$work/add-state-$add_index" ||
            broray_routes_router_export_error \
                "Добавление $destination не сошлось в running-config."
        ...
        broray_routes_router_export_transaction_write \
            ...
            false \
            false \
            "Маршрут $destination создан и подтверждён running-config." ||
            broray_routes_router_export_error \
    ...
    echo "Совпадения на других интерфейсах: $other_matches"
    echo "Конфигурация сохранена: true"
    echo "Проверка создания: bounded convergence show running-config"
