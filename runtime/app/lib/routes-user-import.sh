...
broray_user_routes_preview()
{
    ...
    while [ "$index" -lt "$file_count" ]; do
        ...
        text_probe_file="$work/text-probe-$index.bin"
        ...
        # BusyBox 1.37 does not implement GNU od -A/-t.  Strip NUL bytes into
        # a private peer file, check the producer explicitly, then require
        # byte equality.  A failed producer or comparison is rejected closed;
        # no pipeline status can mask the binary-input decision.
        if ! tr -d '\000' <"$raw_file" >"$text_probe_file"; then
            broray_user_routes_fail \
                "FILE_NOT_TEXT" \
                "Не удалось безопасно подтвердить текстовый формат файла $original_name." || return 1
        fi
        if ! cmp -s "$raw_file" "$text_probe_file"; then
            broray_user_routes_fail \
                "FILE_NOT_TEXT" \
                "Файл $original_name содержит нулевые байты и не является текстовым BAT-файлом." || return 1
        fi
        rm -f "$text_probe_file"
    ...
    case "$interface" in
        ...
        *)
            broray_user_routes_fail \
                ...
                "Некорректный управляемый интерфейс Keenetic." || return 1
            ;;
    esac
