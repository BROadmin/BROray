...
{
    broray_route_resource_acquire "$BRORAY_ROUTES_EXPORT_LOCK" "build-export" "${BRORAY_ROUTES_EXPORT_BUNDLE:-}" || return $?
    BRORAY_ROUTES_EXPORT_LOCK_TOKEN="$BRORAY_ROUTE_RESOURCE_TOKEN"
    return 0
}

broray_routes_export_lock_release()
{
    broray_route_resource_release "$BRORAY_ROUTES_EXPORT_LOCK" "${BRORAY_ROUTES_EXPORT_LOCK_TOKEN:-}" || return $?
    BRORAY_ROUTES_EXPORT_LOCK_TOKEN=''
    return 0
}
...
broray_routes_export_build_run()
{
    ...
    case "$managed_interface" in
        Proxy[0-9]*) ;;
        *) broray_routes_export_error "Некорректный управляемый интерфейс Keenetic." ;;
    esac
    case "${managed_interface#Proxy}" in
        ''|*[!0-9]*) broray_routes_export_error "Некорректный управляемый интерфейс Keenetic." ;;
    esac
    ...
    trap 'broray_routes_export_cleanup' EXIT
    trap 'exit 129' HUP
    trap 'exit 130' INT
    trap 'exit 143' TERM
