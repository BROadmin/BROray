...
broray_routes_registry_migrate()
{
    ...
    registry_ids="whatsapp youtube chatgpt facebook instagram meta tiktok speedtest wikipedia"
    registry_interface="$(jq -r '.managedInterface // empty' "$registry_routes/config.json" 2>/dev/null)"

    case "$registry_interface" in
        Proxy[0-9]*) ;;
        *) broray_routes_registry_error "Некорректный управляемый интерфейс Keenetic."; return 1 ;;
    esac
    case "${registry_interface#Proxy}" in
        ''|*[!0-9]*) broray_routes_registry_error "Некорректный управляемый интерфейс Keenetic."; return 1 ;;
    esac
    ...
    do
        ...
        jq --arg target_interface "$registry_interface" '
