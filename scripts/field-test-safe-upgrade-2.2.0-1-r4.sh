...
TMP_ROOT="${BRORAY_TMP_ROOT:-/opt/tmp}"
BRORAY_TMP_ROOT="$TMP_ROOT"
export BRORAY_TMP_ROOT
...
SOURCE_IPK="${1:-$TMP_ROOT/broray-2.2.0-1-field-test/broray_2.2.0-1_aarch64-3.10.ipk}"
...
say "Используется локальный IPK без создания второй копии: $SOURCE_IPK"
...
free_after_backup="$(free_opt_kb)"
case "$free_after_backup" in
    ''|*[!0-9]*)
        say "ОШИБКА: после резервного копирования не удалось определить свободное место"
        BACKUP_READY=false
        cleanup_work
        finish 1
        ;;
esac
if [ "$free_after_backup" -lt "$required_kb" ]; then
    say "ОШИБКА: после резервного копирования свободно $free_after_backup КБ, требуется $required_kb КБ"
    say "Рабочая установка не изменялась; временная копия удаляется."
    BACKUP_READY=false
    cleanup_work
    finish 1
fi
say "После резервного копирования свободно: $free_after_backup КБ — достаточно"
printf '%s\n%s\n' "$BACKUP" "$(date '+%s')" >"$MARKER" ||
    fail_update "не удалось передать резервную копию OPKG"
say "Устанавливается проверенный локальный IPK..."
BRORAY_TMP_ROOT="$TMP_ROOT" BRORAY_OPKG_BACKUP="$BACKUP" \
