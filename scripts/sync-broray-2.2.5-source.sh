...
work="${RUNNER_TEMP:-/tmp}/broray-2.2.5-source-sync"
rm -rf "$work"
...
curl -fL --retry 5 --retry-delay 2 \
  ...
  "$SOURCE_URL?run=${GITHUB_RUN_ID:-manual}-${GITHUB_RUN_ATTEMPT:-1}"
...
for file in \
  ...
  curl -fL --retry 5 --retry-delay 2 \
    ...
    -H 'Cache-Control: no-cache, no-store' \
    -o "scripts/$file" \
    "https://api.brovibe.cloud/releases/$file?run=${GITHUB_RUN_ID:-manual}-${GITHUB_RUN_ATTEMPT:-1}"
...
curl -fL --retry 5 --retry-delay 2 \
  ...
  "https://api.brovibe.cloud/releases/BROray-2.2.5-release-manifest.json?run=${GITHUB_RUN_ID:-manual}-${GITHUB_RUN_ATTEMPT:-1}"
...
Ветка релиза синхронизирована с точным source overlay опубликованного и
установленного пакета BROray 2.2.5. Бинарный файл Xray намеренно не хранится
в Git: он загружается из официального Xray-core и проверяется по закреплённой
SHA-256.
...
rm -f \
  ...
  .github/workflows/sync-broray-2.2.5-on-pr.yml \
  scripts/sync-broray-2.2.5-source.sh
...
while IFS= read -r file; do jq -e . "$file" >/dev/null; done < <(find root/opt/broray -type f -name '*.json' | sort)
while IFS= read -r file; do node --check "$file" >/dev/null; done < <(find root/opt/broray/web-new/assets/js root/opt/broray/web-src/assets/js -type f -name '*.js' | sort)

scripts/build-webui.sh
diff -qr --exclude BUILD --exclude SOURCE-MIRROR.md root/opt/broray/web-src root/opt/broray/web-new

if grep -RInE '(vless|vmess|trojan|hysteria2)://|[0-9a-fA-F]{8}-[0-9a-fA-F]{4}-[1-5][0-9a-fA-F]{3}-[89abAB][0-9a-fA-F]{3}-[0-9a-fA-F]{12}' root docs release scripts packaging --exclude='*.svg'; then
