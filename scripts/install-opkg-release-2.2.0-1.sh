...
WORK="$TMP_ROOT/broray-install-2.2.0-1-$$"
IPK="$WORK/broray_2.2.0-1_aarch64-3.10.ipk"
...
health() {
    ...
    [ -x "$OPT_ROOT/broray/bin/broray-routes-dot" ] || return 1
    [ -x "$OPT_ROOT/broray/lib/broray-cleanup.sh" ] || return 1
    [ -x "$OPT_ROOT/broray/web-new/api/broray/cleanup.cgi" ] || return 1
    [ -r "$OPT_ROOT/broray/web-new/assets/images/support/cloudtips-qr.svg" ] || return 1
}
...
rm -rf "$OPT_ROOT/var/cache/opkg" "$OPT_ROOT/var/opkg-lists"
...
[ "$PACKAGE_SHA" != "d6df5cf179b66e7f1867071db6f649d584dda3eac17df630e70ec23e38a09f07" ] || fail "установщик не подготовлен к публикации"
...
echo "Все пять служб, маршруты, DNS-over-TLS и WebUI: OK"
