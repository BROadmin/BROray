...
`r14c68` является проверенной базовой установкой. После успешной загрузки откройте **BROray → Проверить обновление → Установить обновление**, чтобы перейти на текущий Stable `3.0.1-r09` универсальным обновителем. Этот путь использует общий алгоритм обновления без ручной коррекции файлов, прав и канала.

После обновления обязательно очистите кэш браузера и заново откройте WebUI. На компьютере используйте `Ctrl+R`; на мобильном устройстве очистите кэш сайта и перезагрузите страницу.
...
- отдельная публикация bootstrap не изменяет проверенные immutable-байты базовой версии `3.0.0-r14c68` и текущий Stable-канал;
