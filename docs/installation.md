...
Успешная чистая установка содержит `UPDATE_DECISION=clean-install`, `CLEAN_BOOTSTRAP=PASS`, `INTERMEDIATE_APPLICATION_RELEASE=NONE` и завершается строками `BRORAY_INSTALL=PASS`, `CANDIDATE=3.0.1-r08c01`, `STABLE=YES`. Частично установленное или неоднозначное состояние отклоняется без обхода защитных проверок.

## Альтернативная установка через штатный механизм Keenetic

Отдельный подписанный bootstrap позволяет поставить проверенную базовую версию `r14c68` через каталог `/opt/install/*.tgz`. Это альтернативный вариант, когда архив удобнее перенести на накопитель роутера через SFTP или USB, а запуск выполнить штатной перезагрузкой Keenetic. В отличие от обычной терминальной установки, после загрузки этого отдельного архива требуется обновление через WebUI до текущего Stable `3.0.1-r08`.
...
Для текущего релиза ожидаются release `3.0.1-r08`, кандидат `3.0.1-r08c01`, WebUI `WebUI-3.0.1-r08c01`, пакет OPKG `3.0.0-r14`, `broray-updater/5`, канал `stable` и отсутствие доступного обновления.

Прямая чистая установка и отсутствие промежуточного приложения подтверждены для архитектуры текущего Stable. Для `r08c01` физически проверены все обязательные службы, Xray, семь серверов, три DNS-over-TLS-записи, 12 маршрутов Wikipedia, KeenDNS, все страницы и кнопки WebUI. Отдельно проверено обновление из состояния, где собственный исправный ProxyN расходился с активным сервером только отображаемым именем: синхронизация завершилась методом `sync-name`, полный повторный контроль прошёл, откат не выполнялся. Расхождения адреса, протокола или подтверждения владения этим исключением не принимаются. Универсальный переход от `3.0.0-r1` и updater-v5 не изменялись.
