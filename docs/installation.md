...
INSTALLED_RELEASE=3.0.1-r02
CURRENT_RELEASE=3.0.1-r02
...
Успешная чистая установка содержит `UPDATE_DECISION=clean-install`, `CLEAN_BOOTSTRAP=PASS`, `INTERMEDIATE_APPLICATION_RELEASE=NONE` и завершается строками `BRORAY_INSTALL=PASS`, `CANDIDATE=3.0.1-r02c05`, `STABLE=YES`. Частично установленное или неоднозначное состояние отклоняется без обхода защитных проверок.

## Альтернативная установка через штатный механизм Keenetic

Отдельный подписанный bootstrap позволяет поставить проверенную базовую версию `r14c68` через каталог `/opt/install/*.tgz`. Это альтернативный вариант, когда архив удобнее перенести на накопитель роутера через SFTP или USB, а запуск выполнить штатной перезагрузкой Keenetic. В отличие от обычной терминальной установки, после загрузки этого отдельного архива требуется обновление через WebUI до текущего Stable `3.0.1-r02`.
...
Для текущего релиза ожидаются release `3.0.1-r02`, кандидат `3.0.1-r02c05`, WebUI `WebUI-3.0.1-r02c05`, пакет OPKG `3.0.0-r14`, `broray-updater/5`, канал `stable` и отсутствие доступного обновления.

Физический контур `r02c05` начат с полного штатного удаления без архива и подтвердил прямую чистую установку целевого приложения, отсутствие промежуточного app-slot, здоровую OPKG-регистрацию, Xray, Proxy0, updater, монитор и восемь страниц WebUI. Повторный запуск публичного Stable-установщика завершился без мутации. Универсальный переход от `3.0.0-r1` и updater-v5 не изменялись.
...
После завершения обновления обязательно очистите кэш браузера и заново откройте WebUI. На компьютере выполните жёсткое обновление страницы сочетанием `Ctrl+F5`; на мобильном устройстве очистите кэш сайта в настройках браузера и перезагрузите страницу.
