...
INSTALLED_RELEASE=3.0.0-r20
CURRENT_RELEASE=3.0.0-r20
...
Отдельный подписанный bootstrap позволяет поставить проверенную базовую версию `r14c68` через каталог `/opt/install/*.tgz`. Этот вариант полезен, когда архив удобнее перенести на накопитель роутера через SFTP или USB, а запуск выполнить штатной перезагрузкой Keenetic. После загрузки выполните обычное обновление через WebUI до текущего Stable `r20`.
...
Для текущего релиза ожидаются release `3.0.0-r20`, кандидат `3.0.0-r20c01`, WebUI `WebUI-3.0.0-r20c01`, пакет OPKG `3.0.0-r14`, `broray-updater/5`, канал `stable` и отсутствие доступного обновления.

Физический контур для этого релиза проверен полностью: полное удаление через терминал и WebUI, чистая установка, установка точного `r14c68`, универсальный переход на `r20c01`, DoT, переустановка, все страницы WebUI, перезагрузка и повторный запуск публичного установщика без мутации. Updater-v5 при выпуске не изменялся.
