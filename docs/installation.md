...
INSTALLED_RELEASE=3.0.1-r01
CURRENT_RELEASE=3.0.1-r01
...
Для текущего релиза ожидаются release `3.0.1-r01`, кандидат `3.0.1-r01c03`, WebUI `WebUI-3.0.1-r01c03`, пакет OPKG `3.0.0-r14`, `broray-updater/5`, канал `stable` и отсутствие доступного обновления.

Физический контур `r01c03` проверил автоматическое восстановление монитора, атомарное состояние соединения, итоговый снимок качества, маршрут Wikipedia, управление KeenDNS-адресом, подписки, DoT, Xray, страницы WebUI и повторный запуск публичного Stable-установщика без мутации. Отдельный базовый контур `r14c68` и универсальный переход были пройдены ранее; updater-v5 и clean bootstrap в `r01c03` побайтово не изменялись.
