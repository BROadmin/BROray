# Установка и обслуживание BROray 3.1.0
...
> [!TIP]
> Если вы впервые работаете с Entware или SSH, начните с [пошаговой инструкции для начинающих](beginner-installation.md). В ней отдельно разобраны Entware, Termius/PuTTY, чистая установка, обновление, переустановка и удаление.
...
Успешная чистая установка содержит `UPDATE_DECISION=clean-install`, `CLEAN_BOOTSTRAP=PASS`, `INTERMEDIATE_APPLICATION_RELEASE=NONE` и завершается строками `BRORAY_INSTALL=PASS`, `CANDIDATE=3.1.0-r09c02`, `STABLE=YES`. Частично установленное или неоднозначное состояние отклоняется без обхода защитных проверок.

## Альтернативная установка через штатный механизм Keenetic

Отдельный подписанный bootstrap позволяет поставить проверенную базовую версию `r14c68` через каталог `/opt/install/*.tgz`. Это альтернативный вариант, когда архив удобнее перенести на накопитель роутера через SFTP или USB, а запуск выполнить штатной перезагрузкой Keenetic. В отличие от обычной терминальной установки, после загрузки этого отдельного архива требуется обновление через WebUI до текущего Stable `3.1.0-r09`.
...
Для текущего релиза ожидаются release `3.1.0-r09`, кандидат `3.1.0-r09c02`, WebUI `WebUI-3.1.0-r09c02`, пакет OPKG `3.0.0-r14`, `broray-updater/5`, канал `stable` и отсутствие доступного обновления.

Для `3.1.0-r09c02` физически подтверждена чистая установка без промежуточного приложения с автоматической установкой Xray **26.9.9**, проверены компоненты и основные действия WebUI. Матрица шести разрешённых версий и 13 групп WebUI-проверок выполнена на `r09c01`; функциональный код конечного `r09c02` не изменён. Полный объём и ограничения: [заметки к релизу 3.1.0](RELEASE-3.1.0.md).

## Выбор версии Xray

При чистой установке устанавливается **26.9.9**. **Обновление BROray сохраняет имеющуюся версию Xray.** Чтобы изменить её отдельно:

1. Откройте страницу **Xray** и нажмите **Проверить версии**.
2. Выберите нужную версию. Для предварительных выпусков включите соответствующий переключатель.
3. Посмотрите статус совместимости с текущей сборкой BROray. Модель роутера для этого статуса не используется.
4. Нажмите **Установить** и подтвердите предупреждения, если они показаны. Понижение требует отдельного подтверждения.
5. Дождитесь завершения и проверьте версию и состояние Xray. Для повторной установки используйте **Переустановить текущую версию**.

В реестре совместимы 26.3.27, 26.6.27, 26.7.11, 26.7.28, 26.9.8 и 26.9.9. **26.2.6 несовместима и недоступна для установки.** Отсутствие проверки обозначается отдельным статусом, а не считается совместимостью. При неудачной замене предусмотрен возврат прежнего ядра; не закрывайте страницу до получения результата.

После установки с нуля сначала добавьте и активируйте сервер. Без него начальная конфигурация не предоставляет VPN-соединение. В диагностике может сохраняться предупреждение об отсутствии прежней записи «Испытание на Keenetic»; оно отдельно от нового реестра совместимости.
