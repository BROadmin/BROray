...
INSTALLED_RELEASE=3.0.0-r23
CURRENT_RELEASE=3.0.0-r23
...
Отдельный подписанный bootstrap позволяет поставить проверенную базовую версию `r14c68` через каталог `/opt/install/*.tgz`. Этот вариант полезен, когда архив удобнее перенести на накопитель роутера через SFTP или USB, а запуск выполнить штатной перезагрузкой Keenetic. После загрузки выполните обычное обновление через WebUI до текущего Stable `r23`.
...
Для текущего релиза ожидаются release `3.0.0-r23`, кандидат `3.0.0-r23c02`, WebUI `WebUI-3.0.0-r23c02`, пакет OPKG `3.0.0-r14`, `broray-updater/5`, канал `stable` и отсутствие доступного обновления.

Физический контур `r23c02` проверил обычное WebUI-обновление с `r22c01`, сохранение серверов и активного выбора, расписание автоматической проверки качества, отрицательные API-сценарии, DoT, подписки, Xray, 12 страниц, 9 read-only API, перезагрузку и повторный запуск публичного Stable-установщика без мутации. Отдельный базовый контур `r14c68` и универсальный переход были пройдены ранее; updater-v5 и clean bootstrap в `r23c02` побайтово не изменялись.
