...
**BROray 3.0.0-r20 опубликован в Stable 30 августа 2026 года.**

- публичный релиз: `3.0.0-r20`;
...
Stable создан из физически проверенных байтов `r20c01` без пересборки приложения и без изменения updater-v5 при публикации. Универсальный обновитель принимает решение только по установленному и текущему `releaseId`: более старая версия обновляется, равная ничего не меняет, более новая не понижается. Версия OPKG и история кандидатов в решении не участвуют. В `r20c01` исправлено полное удаление чистой базовой установки `r14c68`: если старый receipt отсутствует, BROray восстанавливает исходные файлы Lighttpd только по точным известным хешам и возвращает исполняемый режим init-скрипта `0755`. Полные контуры «чистая установка → `r14c68` → универсальное обновление» и «обновление с отдельной `r14c68`» прошли на физическом Keenetic через терминал и обычный WebUI.
...
| SHA-256 Stable index | `b87deb3cca0eb01b1632aa8ceb1bcb81de6972bee5b7925e694088cffa9a2ebd` |

Технический идентификатор `r20c01` нужен для проверки происхождения байтов. OPKG остаётся `3.0.0-r14` по архитектуре компактного app-slot и не ограничивает обновление приложения.
...
[ "$(sha256sum "$T" | awk 'NR==1{print $1}')" = \
  '3c3faa352ef78821de791a3e932602b88efadf9b82fecde39c535a67bfbb14c0' ]
sh "$T"
)
```
...
Этот проверенный bootstrap устанавливает базовый `r14c68`. После первого входа откройте **BROray → Проверить обновление → Установить обновление**, чтобы перейти на текущий `r20`. Установщик проверяет архитектуру, компоненты, свободное место и контрольные суммы. После успешной установки WebUI доступен напрямую по адресу `http://<LAN-IP-роутера>:8080/`.
...
Для устройств, где удобнее использовать каталог автозапуска Entware, сохранён отдельный подписанный bootstrap базовой установки `3.0.0-r14c68` через штатный механизм `/opt/install/*.tgz`. После загрузки откройте WebUI и обновитесь до текущего Stable `3.0.0-r20`.
...
Эта документация описывает опубликованный Stable `3.0.0-r20`, полученный из технического кандидата `r20c01`. Публичные release-артефакты побайтово привязаны к физически проверенному кандидату; исходное дерево GitHub остаётся отдельным контуром и не объявляется тождественным опубликованному архиву без специальной проверяемой синхронизации.
