...
- исходный технический кандидат: `3.0.0-r14c67`;
- статус: **Stable**;
...
Stable создан из проверенных байтов `r14c67` без изменения приложения или платформы при публикации. Предыдущий Stable сохранён на сервере как резервная точка; откат при выпуске не потребовался.
...
| SHA-256 Stable index | `cc0f73b150ccad00dbac16acf1bdf0c2ae4af11ed49b7e8879254536e6180e09` |

Технический идентификатор `r14c67` нужен для проверки происхождения байтов. Пользовательская версия приложения — `3.0.0`, версия пакета — `3.0.0-r14`.
...
Эта документация описывает опубликованный Stable `3.0.0-r14`, полученный из технического кандидата `r14c67`. Обновление документации само по себе не означает побайтового совпадения текущего `main` с опубликованными архивами: перенос точного release-source overlay и создание тега должны проходить отдельной проверяемой операцией.
