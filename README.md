...
**BROray 3.0.0-r17 опубликован в Stable 29 августа 2026 года.**

- публичный релиз: `3.0.0-r17`;
...
Stable создан из физически проверенных байтов `r17c01` без пересборки приложения и updater platform при публикации. Универсальный обновитель принимает решение только по установленному и текущему `releaseId`: более старая версия обновляется, равная ничего не меняет, более новая не понижается. Версия OPKG и история кандидатов в решении не участвуют. В `r17c01` исправлены три причины отказа чистого перехода: verifier Minisign устанавливается исполняемым, выбранный Stable-канал сохраняется, устаревший BROray OPKG-feed `r14cNN` удаляется. Полный контур «полное удаление → чистый `r14c68` → автоматическое обновление до `r17c01`» прошёл на физическом Keenetic без ручного ремонта.
...
| SHA-256 Stable index | `432d1ae97cec76abd7913cb20187eaad1a34b52b0fe098e667efe1b67a99b20a` |

Технический идентификатор `r17c01` нужен для проверки происхождения байтов. OPKG остаётся `3.0.0-r14` по архитектуре компактного app-slot и не ограничивает обновление приложения.
...
Для устройств, где удобнее использовать каталог автозапуска Entware, сохранён отдельный подписанный bootstrap базовой установки `3.0.0-r14c68` через штатный механизм `/opt/install/*.tgz`. После загрузки откройте WebUI и обновитесь до текущего Stable `3.0.0-r17`.
