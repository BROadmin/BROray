...
broray_detect_keendns_fqdn() {
    KEENDNS_STATUS="$(ndmc -c 'show ndns' 2>/dev/null)" || return 1
    ...
    [ -n "$KEENDNS_NAME" ] || return 1
    [ -n "$KEENDNS_DOMAIN" ] || return 1

    case "$KEENDNS_NAME" in
        *[!A-Za-z0-9.-]*)
            return 1
            ;;
    esac

    case "$KEENDNS_DOMAIN" in
        *[!A-Za-z0-9.-]*)
            return 1
            ;;
    esac
...
broray_detect_webui_urls() {
    ...
    KEENDNS_FQDN="$(broray_detect_keendns_fqdn 2>/dev/null)"

    if [ -n "$KEENDNS_FQDN" ]; then
        printf 'https://broray.%s/\n' "$KEENDNS_FQDN"
    fi
}
